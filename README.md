# MMU2S-Klipper
Python serial driver for running the stock MMU2S/3 firmware on Klipper. Tested on an MMU2S with a Prusa i3 MK3S+. Supports both external USB to Serial adapter and on board connection with the Einsy RAMBo of a MK3.
...
* Free USB port
* 5V USB to Serial Adapter (optional)

## Instructions (Einsy RAMBo Connection)
* Copy ```mmu2s.py``` to the extras folder within klipper.
* Add ```mmu2s.cfg``` to klipper config directory and include it in printer.cfg
    * Ensure that ```connection_method: Native``` is present

### Compiling custom Klipper firmware 
* Copy ```mmu.c``` and ```mmu.h``` to the ```klipper/src/avr``` folder
* Add ```avr/mmu.c``` to the avr ```Makefile```
* Run make menuconfig and ensure these options are selected:
    * Micro-controller Architecture -> Atmega AVR
    * Processor model -> atmega2560
    * Processor speed -> 16Mhz
    * Communication interface -> UART1 (For using PI connection on back of RAMBo board)
    * Baud Rate -> 250000
* Run ```make flash FLASH_DEVICE=/dev/serial/by-id/usb-Prusa_Research__prusa3d.com__Original_Prusa_i3_MK3_CZPX0122X004XK33932-if00``` (ensure that the serial device matches)

## Instructions (USB to Serial Adapter)
...
* Copy ```mmu2s.py``` to the extras folder within klipper.
* Add ```mmu2s.cfg``` to klipper config directory and include it in printer.cfg.
    * Ensure that ```connection_method: Serial``` is present
